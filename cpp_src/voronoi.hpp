...
void Voronoi<DIM>::construct_voronoi_vertices() {
  ...
  for (int v = 0; v < comp.ndcells[2]; v++) {
    ...
    std::vector neighb_cells_vec(neighb_cells.begin(), neighb_cells.end());
...
void Voronoi<DIM>::construct_cell_areas_and_centroids() {
  ...
  for (int c = 0; c < comp.ndcells[0]; c++) {
    ...
    std::transform(neighb_vertices.begin(), neighb_vertices.end(), vert_pos.begin(), [this](int i){return voronoi_vertices.segment<DIM>(DIM*i);});
